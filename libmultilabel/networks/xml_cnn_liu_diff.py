...
class XMLCNNLiuDiff(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        weights = np.load(kwargs['load_weight'])
        embed_vecs = torch.Tensor(weights['embedding_weight'])
        super(XMLCNNLiuDiff, self).__init__(embed_vecs, **kwargs)
        if not filter_sizes:
            raise ValueError(
        ...
        for i in range(3):
            self.convs[i].weight.data = torch.FloatTensor(np.copy(weights[f'conv{i+1}_weight'][:, :, ::-1]))
            self.convs[i].bias.data.fill_(0.)
        self.linear1.weight.data = torch.FloatTensor(np.copy(weights['hidden_weight']).transpose())
        self.linear1.bias.data.fill_(0.)
        self.linear2.weight.data = torch.FloatTensor(np.copy(weights['output_weight']).transpose())
